...
function drawSurface() {
    ...
    for (let j = 0; j <= POINTS; j++) {
        const u = j * 4 * Math.PI / POINTS;
        for (let i = 0; i <= POINTS; i++) {
            const v = i * 2 * Math.PI / POINTS;

            const x = (Math.exp(m * u) + Math.exp(p * u) * Math.cos(v)) * Math.cos(u);
            const y = (Math.exp(m * u) + Math.exp(p * u) * Math.cos(v)) * Math.sin(u);
            const z = Math.exp(p * u) * Math.sin(v)
...
function draw() {
    ...
    const translateToPointZero = m4.translation(0, 0, -20);
 ...
 */
function init() {
    mInput = document.getElementById('numM');
    mSlider = document.getElementById('m');
    pInput = document.getElementById('numP');
    pSlider = document.getElementById('p');
    ...
    eyeSepInput.addEventListener('change', () => draw())
